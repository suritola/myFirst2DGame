...
public class CharacterKit : MonoBehaviour
{
    ...
    float stealthUntil;
    GameObject decoy;
    public bool Stealthed => Time.time < stealthUntil;

    public bool Busy => charging;
    public float AttackSpeedMul => Stealthed ? 1.2f : 1f;
    public float MoveMul => Stealthed ? 1.6f : 1f;
    ...
    void LateUpdate()
    {
        ...
        // 은신: 반투명, 끝나면 원래대로
        Color c = body.color;
        c.a = Stealthed ? 0.35f : 1f;
        body.color = c;
        if (!Stealthed && decoy != null)
        {
            if (EnermyController.Decoy == decoy.transform) EnermyController.Decoy = null;
            Destroy(decoy);
            Fx.Play("fx_stealth", transform.position, 3f, Color.white, 14f);
        }
    }
    ...
    public void Attack()
    {
        ...
        switch (Id)
        {
            case CharacterId.Swordsman:
                // 앞으로 뻗어 나가는 초승달 검기 (지나가는 적 모두, 적마다 한 번)
                foreach (Vector2 d in Spread(dir, shots, 14f))
                {
                    Bullet b = Projectile(start, d, dmg, 999, 32f, def.range, "fx_swordwave", 2.4f, Color.white, false);
                    if (b != null) b.hitOnce = new HashSet<int>();
                }
                Play("slash", 0.6f, 1.1f);
                break;
    ...
    public void UpdateUlt(SkillGauge gauge)
    {
        ...
        switch (Id)
        {
            ...
            case CharacterId.Rogue:
                ...
                {
                    stealthUntil = Time.time + 6f;
                    // 적은 숨은 자리로 몰려가고 플레이어를 놓침
                    decoy = new GameObject("StealthDecoy");
                    decoy.transform.position = transform.position;
                    EnermyController.Decoy = decoy.transform;
                    Fx.Play("fx_stealth", transform.position, 4f, Color.white, 14f);
                    Play("whoosh", 0.8f, 0.7f);
                    Spend(gauge);
                }
                break;
            case CharacterId.Archer:
