...
| | |
|---|---|
| 🧙 **캐릭터 5종** | 거너 · 검사 · 도적 (기본) · 궁수 · 연금술사 (포인트 구매), 캐릭터마다 다른 평타(검사는 장검으로 직접 베기) · 캐릭터 무기에 맞춘 레벨업 카드 · 캐릭터별 능력치 상점 · 우클릭(도적은 무적 출혈 돌진) · 특수 능력 8종(전용 무기마다 우클릭 궁극기) · 시작/엔딩 이야기, 10칸 도감 |
| 🔫 **리볼버 사격** | 거너: 마우스로 조준하는 8연발 사격. 탄창이 비면 자동 재장전 |
| 🎯 **타겟팅 스킬** | 시간이 지나면 차는 게이지로 적을 조준 → **들고 있는 무기마다 다른 조준 화면과 필살기** |
| 🗺️ **세 개의 스테이지** | 1장 지하 묘역 → 2장 불타는 지옥 → 3장 초원 (모두 넓은 맵, 통과 가능한 장식만) |
| 🎚️ **난이도** | 쉬움 · 보통 · 어려움 — 쉬움을 클리어하면 보통 · 어려움이, 어려움을 클리어하면 **무한 모드**가 열림 |
| ♾️ **무한 모드** | 불타는 사막에서 세 장의 적이 모두 나오고 세 보스가 번갈아 등장, 생존 시간 최고 기록 |
| 🎬 **이야기** | 시작 연출(주인공이 신전 아래로 떨어짐), 보스 등장 연출(카메라 이동 · 보스 대사 · 목소리), 엔딩 이야기 — `Space` 대사 넘기기 · `ESC` 건너뛰기 |
| ⚠️ **피할 수 있는 스킬** | 적마다 점프 · X자 레이저 · 장판 등 서로 다른 스킬, 전부 **경고 표시 후 발동** |
| 👑 **보스전** | 리치 왕 · 지옥의 군주 · 분열하는 킹 슬라임, 각자 스킬 5개 + 체력 절반 아래 특수 스킬 |
| 🌳 **특수 능력 20종** | 무기 · 스킬 · 패시브를 스킬 트리에서 고르고, 중간 보스 보상으로 **진화** |
| 🎨 **도트 이펙트** | 폭발 · 번개 · 레이저 · 룬 마법진 · 화염 회오리 · 유령 낫 · 영혼 폭발 · 빛기둥 등 직접 그린 도트 이펙트 24종 |
| 🎵 **배경음악** | 메뉴 · 1장 · 2장 · 3장 · 사막(무한) · 보스전 음악을 직접 합성, 보스가 나오면 긴박한 곡으로 전환 |
| 🌐 **4개 언어** | 한국어 · English · 日本語 · 中文, 볼륨 · 화면 · 키 설정 (설정 · 튜토리얼 메뉴) |
| 🛒 **떠돌이 상점** | 적 35마리마다 나타나는 상점 제단에 10초 안에 다가가 `Space`로 쇼핑, 필살기 강화 창 포함 |
...
| `E` / `F` / `C` | 특수 스킬 1 / 2 / 3 — 대시·장판·갈고리는 누르고 있으면 미리보기, 떼면 발동 |
| `T` | 특수 강화 (특수 능력 포인트가 있을 때) |
| `Space` (상점 제단 앞) | 상점 열기 |
| 레벨업 창 | 카드를 **클릭**해서 고르고 **`Space`**로 확정 |
| `ESC` | 일시정지 메뉴 (설정 버튼 포함, 필살기 조준 중에도 가능) / 상점 · 설정 닫기 |

키보드 키는 **설정 → 조작**에서 바꿀 수 있습니다 (위 표는 기본값). 게임 창이 포커스를 잃으면 자동으로 일시정지됩니다.

## ⚙️ 메뉴 · 설정

- **메인 메뉴**: **난이도**(쉬움 · 보통 · 어려움 · 무한, 잠긴 난이도는 회색) · 게임 시작 · **캐릭터** · **튜토리얼** · **도감** · **설정** · 게임 종료
- **캐릭터**: 10칸 도감. 잠긴 캐릭터는 검은 실루엣과 ???, 누르면 거너 대비 스탯(체력 · 공격력 · 공격 속도 · 사거리 · 스킬 게이지(높을수록 늦게 참) · 이동 속도 · 탄창)과 <선택> <취소>, 잠겼으면 <구매>. 포인트는 적을 처치할 때마다 쌓이고 저장됨
...
- **설정** (메뉴 또는 게임 중 `ESC` → 설정)
  - **일반**: 언어(한국어 기본 · English · 日本語 · 中文), 전체 볼륨(기본 50%) · 음악 · 효과음
  - **화면**: 전체 화면 / 창 모드, 해상도, 수직 동기화, 화면 흔들림, 번쩍임 효과
  - **조작**: 이동 · 재장전 · 무기 교체 · 스킬 1~3 · 상호작용 · 특수 강화 키 바꾸기 (겹치면 서로 맞바꿈, 기본값 복원)
  - 설정은 저장되어 다음 실행에도 유지됩니다. 일본어 · 중국어는 [Noto Sans CJK](Assets/Resources/Fonts/OFL-NotoSansCJK.txt)(OFL) 폰트를 씁니다.
...
- **적 피해 숫자**: 적 · 보스를 때리면 머리 위로 피해 숫자가 떠오름 (연달아 들어간 피해는 합쳐서, 큰 피해는 크고 주황색)
...
기본 권총 · 영혼 인도 지팡이 · 번개 사슬총 · 부메랑 낫은 우클릭을 누르고 있으면 시간이 느려지며 **무기에 맞는 조준 화면**이 나오고, 떼면 필살기가 나갑니다.
산탄총 · 쌍권총 · 유탄 발사기는 조준이 필요 없어서 **우클릭을 누르는 즉시** 발동합니다. 화염 방사기(회오리 위치)와 저격총(레일건 방향)은 우클릭을 누른 채 조준합니다.

| 들고 있는 무기 | 조준 화면 | 필살기 |
|---|---|---|
| 기본 권총 | 적마다 조준 표식 (기본 6마리) | **일제 사격** — 조준한 적에게 차례로 강한 총알 |
| 지옥불 산탄총 | 마우스 쪽으로 펼쳐지는 불꽃 부채꼴, 안에 든 적에 조준경 | **지옥불 포격** — 거대한 부채꼴 폭발 3연발 |
| 영혼 저격총 | 충전될수록 굵어지는 레이저 조준선, 선 위의 적마다 조준경 | **관통 레일건** — 화면을 가로지르는 굵은 광선 (선 위의 적이 많을수록 강함) |
| 저주받은 쌍권총 | 플레이어 주위를 도는 금빛 룬 고리와 총구 섬광 | **총알 폭풍** — 1.6초 동안 두 줄기 나선으로 사방 난사 |
| 화염 방사기 | 마우스 위치에 불꽃 룬과 회오리 미리보기, 빨려 들어갈 범위 | **화염 회오리** — 불기둥이 적에게 다가가며 빨아들이고 태움 |
| 영혼 인도 지팡이 | 주위에 모여드는 영혼 구슬, 적마다 영혼 표식 | **영혼 떼** — 구슬 다섯이 번갈아 달려듦 |
| 번개 사슬총 | 머리 위에서 커지는 먹구름, 적마다 번개 표식과 이어지는 번개 | **뇌운** — 2.5초 동안 번개가 연달아 떨어짐 |
| 부메랑 낫 | 적마다 해골 표식과 베어 나갈 순서의 경로선 | **죽음의 춤** — 무적 상태로 적 사이를 순간이동하며 벰 |
| 용암 유탄 발사기 | 마우스 쪽으로 떨어질 운석 자리 격자 | **용암 융단폭격** — 줄지어 운석이 떨어져 폭발 |
...
무기를 여러 개 고르면 `Q`로 돌아가며 바꾸고, 스킬은 고른 순서대로 `E` · `F` · `C`에 배정됩니다.

#### ⬆️ 특수 강화와 진화

중간 보스와 3장 입장으로 얻은 **특수 능력 포인트**는 화면 아래 반짝이는 **특수 강화** 버튼(또는 `T`)으로 씁니다.

- **새 능력**을 누르면 배웁니다 (스킬은 `E` · `F` · `C` 세 칸까지).
...
| 영혼 저격총 | 누르고 있다 떼면 발사 (누르는 동안 화면 끝까지 점선 조준선), 모든 적 관통, 충전에 따라 150~300% (탄창 4) |
...
<details>
<summary>✨ 스킬 8종 — <code>E</code> · <code>F</code> · <code>C</code>로 사용</summary>
...
상점은 아무 때나 열 수 없습니다. **적을 35마리 잡을 때마다** 플레이어 근처에 **상점 제단**이 나타나고, **10초 안에 다가가면** 말풍선에 `[SPACE] 상점 열기`가 뜹니다. 이때 `Space`를 누르면 상점이 열려요 (시간이 지나면 사라집니다).
...
기본 권총과 보유한 특수 무기마다 **필살기**를 따로 강화합니다.

| 강화 | 한 단계 효과 | 최대 | 가격 |
|---|---|:---:|---|
| 위력 | 필살기 피해 +20% | 5 | 60코인부터 ×1.5 |
| 특성 | 권총 · 낫 타겟 수 +2 · 산탄총 포격 +1회 · 저격총 광선 굵기 +30% · 쌍권총 지속 +0.4초 · 화염 회오리 지속 +1.5초 · 지팡이 구슬 +1 · 사슬총 뇌운 지속 +0.8초 · 유탄 폭격 줄 +2 | 3 | 90코인부터 ×1.7 |

#### 🔧 무기 강화 (2장부터, 상점 오른쪽 위 탭)
...
├── Resources/
│   ├── FX/               # 도트 이펙트 스프라이트 시트
│   ├── Music/            # 직접 합성한 배경음악 5곡
│   └── Fonts/            # 일본어 · 중국어 폰트 (Noto Sans CJK, OFL)
├── Scripts/              # 게임 로직 (C#)
...
│   ├── Fx.cs                 # 도트 이펙트 재생
│   ├── Shop.cs / ShopStall.cs            # 상점, 떠돌이 상점 제단 (Space로 열기)
│   ├── WeaponUpgradeShop.cs / SkillUpgradeShop.cs  # 무기 강화 · 필살기 강화 창
│   ├── WeaponAim.cs          # 무기별 타겟팅 조준 화면
│   ├── BulletGlow.cs         # 총알 빛 무리 · 꼬리
│   ├── MusicManager.cs       # 배경음악 (Resources/Music)
│   ├── Loc.cs / LocTable.cs / GameSettings.cs  # 다국어 번역표 · 설정 저장
│   ├── MenuExtras.cs         # 메뉴 튜토리얼 · 설정 창 (일반 · 화면 · 조작)
│   ├── KeyBindings.cs        # 키 설정 저장 · 화면 글자의 키 이름 바꾸기
│   ├── GameMode.cs           # 난이도 (쉬움 · 보통 · 어려움 · 무한) · 잠금 해제 · 수치 배율
│   ├── StoryDirector.cs      # 시작 연출 · 이야기 · 엔딩 (건너뛰기)
│   ├── EndlessMode.cs        # 무한 모드: 불타는 사막 맵 · 보스 주기 등장 · 기록
│   ├── Juice.cs              # 적 처치 · 레벨업 · 코인 획득 이펙트와 소리
│   ├── PlayerLook.cs         # 손에 든 무기 모습 (Resources/Weapons) · 무기별 발사 연출
│   ├── CharacterData.cs      # 캐릭터 10칸 정의 · 거너 대비 스탯 · 잠금 해제 · 포인트 저장
│   ├── CharacterKit.cs       # 거너가 아닌 캐릭터의 몸 그림 · 평타 · 우클릭 스킬
│   ├── CharacterUI.cs        # 메인 메뉴 캐릭터 도감 (선택 · 구매)
│   ├── SpecialAbilities.Kits.cs  # 캐릭터별 특수 능력 32종 (ID 20~51)
...
├── Resources/Desert/     # 불타는 사막 타일 · 소품 (tools/pixelart 로 만듦)
└── Resources/FX/         # 도트 이펙트 시트
Assets/Editor/BuildScript.cs  # 명령줄 윈도우 빌드
tools/release.ps1             # 빌드 → zip → 비공개 저장소 Release 업로드
tools/pixelart/               # 도트 그림 생성 (사막 맵 · 새 이펙트), 외부 라이브러리 없음
tools/audio/                  # 배경음악 합성 (사막)
